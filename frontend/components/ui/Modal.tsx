...
function CloseButton({ closeHref, onClose }: { closeHref?: string; onClose?: () => void }) {
  if (closeHref) {
    return (
      <Link
        href={closeHref}
...
function ModalOverlay({ closeHref, onClose }: { closeHref?: string; onClose?: () => void }) {
  if (closeHref) {
...
function ModalInner({ title, children, size, closeHref, onClose }: ModalProps) {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      <ModalOverlay closeHref={closeHref} onClose={onClose} />
      <div
        className={cn(
...
export const Modal = memo(function Modal({ isOpen, onClose, closeHref, title, children, size = "md" }: ModalProps) {
  ...
  return createPortal(
    <ModalInner isOpen={isOpen} onClose={onClose} closeHref={closeHref} title={title} size={size}>
      {children}
    </ModalInner>,
    document.body
  );
