...
type ModalState =
  ...
  | { type: "view_milestone"; milestone_id: string }
  | { type: "edit_milestone"; milestone_id: string }
  | { type: "delete_milestone"; milestone_id: string }
  | null;
...
interface RoadmapContentProps {
  ...
  error?: string;
  success?: string;
  roadmapPermissions: { can_view: boolean; can_create: boolean; can_edit: boolean; can_delete: boolean };
...
export default function RoadmapContent({
  ...
  const selectedMilestone = useMemo(() => {
    if (!modal || modal.type === "create_milestone") return null;
    return milestones?.find((m) => m.id === modal.milestone_id) ?? null;
  }, [modal, milestones]);

  const selectedMilestoneProject = useMemo(() => {
    if (!selectedMilestone) return null;
    return projects?.find((p) => p.id === selectedMilestone.project_id);
  }, [selectedMilestone, projects]);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        {roadmapPermissions.can_create && (
          <Button className="gap-2" onClick={() => setModal({ type: "create_milestone" })}>
      ...
      {(error || success) && (
        <div className={`p-4 rounded-2xl border text-sm flex items-center gap-3 ${error ? 'bg-red-50 border-red-100 text-red-800' : 'bg-emerald-50 border-emerald-100 text-emerald-800'}`}>
          <div className={`h-2 w-2 rounded-full ${error ? 'bg-red-500' : 'bg-emerald-500'}`} />
          {error || success}
        </div>
      )}
              ...
              milestones.map((m) => (
                <div key={m.id} className="group relative p-4 rounded-2xl bg-white border border-soft hover:border-violet-200 hover:shadow-md transition-all">
                  <div className="flex items-start justify-between">
                    <div onClick={() => setModal({ type: "view_milestone", milestone_id: m.id })} className="flex gap-3 min-w-0 cursor-pointer">
      ...
      <Modal
        isOpen={modal?.type === "create_milestone"}
      ...
      {selectedMilestone && (
        <Modal
          isOpen={modal?.type === "view_milestone"}
          onClose={() => setModal(null)}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-soft">
                ...
                <p className="text-sm font-bold text-main">{format(new Date(selectedMilestone.due_date), "MMM d, yyyy")}</p>
              </div>
              <div className="p-4 rounded-2xl bg-slate-50 border border-soft">
            ...
            <div className="flex justify-end gap-3 pt-4">
               ...
               {roadmapPermissions.can_edit && (
                 <Button onClick={() => setModal({ type: "edit_milestone", milestone_id: selectedMilestone.id })}>Edit Milestone</Button>
               )}
            </div>
          </div>
        </Modal>
      ...
      {selectedMilestone && (
        <Modal
          isOpen={modal?.type === "edit_milestone"}
          onClose={() => setModal(null)}
      ...
      {selectedMilestone && (
        <Modal
          isOpen={modal?.type === "delete_milestone"}
          onClose={() => setModal(null)}
        ...
        >
          <div className="space-y-6 text-center">
            ...
            <form action={deleteMilestone} className="flex gap-3">
              <input type="hidden" name="organization_slug" value={orgSlug} />
              <input type="hidden" name="milestone_id" value={selectedMilestone.id} />
              <Button variant="outline" className="flex-1 w-full" onClick={() => setModal(null)}>Cancel</Button>
