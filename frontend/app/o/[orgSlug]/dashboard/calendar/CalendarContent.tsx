...
export default function CalendarContent({
  ...
  const goToPrevMonth = () => { setCurrentMonth(prevMonth); setCurrentYear(prevYear); };
  const goToNextMonth = () => { setCurrentMonth(nextMonth); setCurrentYear(nextYear); };
  const goToToday = () => {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border border-soft shadow-sm">
          <Button variant="ghost" size="icon" className="rounded-xl h-10 w-10" onClick={goToPrevMonth}><ChevronLeft className="h-5 w-5" /></Button>
          <Button variant="outline" className="font-bold border-soft hover:bg-slate-50 rounded-xl" onClick={goToToday}>Today</Button>
          <Button variant="ghost" size="icon" className="rounded-xl h-10 w-10" onClick={goToNextMonth}><ChevronRight className="h-5 w-5" /></Button>
        </div>
      </header>
