...
class TaskService:
    ...
    def list_tasks(
        ...
        for row in rows:
            fallback = {row.get("department_id")} if row.get("department_id") else set()
    ...
    def create_task(supabase: Client, ticket_id: str, payload: TaskCreate, ctx: RequestContext):
        ...
        allowed_project_ids = AccessScopeService.get_accessible_project_ids(supabase, ctx)
        if allowed_project_ids is not None and ticket.data["project_id"] not in allowed_project_ids:
    ...
    def update_task(supabase: Client, task_id: str, payload: TaskUpdate, ctx: RequestContext):
        ...
        if allowed_project_ids is not None:
            ...
            if not task_row.data:
                raise HTTPException(status_code=404, detail="Task not found")
            if task_row.data["project_id"] not in allowed_project_ids:
                raise HTTPException(status_code=403, detail="Forbidden for this project")
